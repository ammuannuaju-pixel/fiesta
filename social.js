...
async function loadChatHistory() {
  ...
  // Fetch all messages sent by this user with room info
  ...
  if (!messages || !messages.length) {
    list.innerHTML = `
      <div class="chat-history-empty">
        You have not sent any messages in a reading room yet.
        Complete a quiz to join one.
      </div>
    `;
    return;
  }
  ...
  messages.forEach(msg => {
    ...
    if (!grouped[roomId]) {
      grouped[roomId] = {
        genre:    msg.reading_rooms?.genre || "Unknown",
        mood:     msg.reading_rooms?.mood  || "Unknown",
        messages: []
      };
    }
    grouped[roomId].messages.push(msg);
  });

  list.innerHTML = Object.entries(grouped).map(([roomId, room]) => `
    <div class="chat-history-room">
      <div class="chat-history-room-header">
        <div class="chat-history-room-title">
          ${escHTML(room.genre)} — ${escHTML(room.mood)}
        </div>
        <div class="chat-history-room-count">
      ...
      <div class="chat-history-messages">
        ${room.messages.map(msg => `
          <div class="chat-history-msg">
            <div class="chat-history-msg-text">${escHTML(msg.content)}</div>
            <div class="chat-history-msg-time">${formatTime(msg.created_at)}</div>
          </div>
        `).join("")}
      </div>
    </div>
